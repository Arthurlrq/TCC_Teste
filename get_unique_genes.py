# CRIA A LISTA DE GENES UNICOS QUE APARECEM PELO MENOS UMA VEZ NA LISTA DE PARES DE GENES
	...
	if 'olfr' not in gene_name:
		...
		for par in pares:
			...
			if gene_name == gene_a or gene_name == gene_b:
				print("o gene " + gene_name + " esta no par " + par + "\n")
				d_genes[gene_name]['aparicoes'] = d_genes[gene_name]['aparicoes'] + 1
				break;
		# REMOVER GENE QUE NAO APARECE
...
for gene in genes_sc:
	...
	if 'olfr' not in gene_name:
		...
		if gene_name in d_genes:
			print('gene ja esta na lista, entao nao faz nada')
		else:
			d_genes[gene_name] = {}
			d_genes[gene_name]['aparicoes'] = 0
